...
public class Tride
{
    ...
    public Tride(int _id,Sprite _icon,string _name,float _maxHp,float _hp,int _damage,int _depence,float _critical,int _moneyUp,
        int _maxCharacter,int _heal,float _luck, float _block, float _miss,int _length,float _infection, float _kidanp, float _rivival)
    {
        id = _id;
        icon = _icon;
        name = _name;
    ...
    public Tride Clone() 
    {
        return new Tride(id,icon,name,maxHp,hp,damage,depence,critical,moneyUp,maxCharacter,heal,luck,block,miss,length,infection,kidnap,rivival);
    }
